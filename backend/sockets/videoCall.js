...
module.exports = (io, socket) => {
  ...
  socket.on('join-room', async ({ roomId }) => {
    ...
    console.log(`Emitting joined-room event to socket ${socket.id} for room ${roomId}`);
    socket.emit('joined-room', roomId);
    ...
    if (updatedClientsInRoom.size === 1) {
      ...
      io.to(roomId).emit('ready', { userId: user.id });
    } else {
      console.log(`Second user in room ${roomId}, emitting init event`);
      socket.emit('init');
    }
  });
  ...
  socket.on('chat-message', ({ roomId, message }) => {
    ...
    io.to(roomId).emit('chat-message', { userId: socket.userId, message });
  });
